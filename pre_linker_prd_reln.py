...
def build_final_json(text, entities, linker_results):
    ...
    ########################################
    # PRODUCT SIMILARITY RULE
    ########################################

    products = [e for e in entities if e["label"] == "product_name"]

    main_product = None

    for p in products:
        if p.get("is_main_product"):
            main_product = p
            break
    ...
    main_id = product_map[main_product["text"]]["id"]

    for p in products:

        if p["text"] == main_product["text"]:
            continue

        sim = safe_cosine(main_product["embedding"], p["embedding"])

        if sim > PRODUCT_SIM_THRESHOLD:
            product_map[p["text"]]["is_related_to"] = main_id
...
def run_pipeline(text, entities):
    ...
    linker_results, entities = link_entities(text, entities)
